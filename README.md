...
| `src/draw.ts` | the video frame, the skeleton as a designed object, and one routed connector from a faulted joint to its measured number. The count, the verdict and the hint are DOM (see `src/main.ts`), not canvas text |
| `src/main.ts` | the view layer: the HUD on the glass, the rep pulse, the reduced-motion branches. It judges nothing |
...
| `scripts/e2e-hints.mjs`, `scripts/e2e-failure-modes.mjs` | hint timelines on synthetic fake cameras and real clips (read off the DOM HUD); the broken-loop / missing-model failure modes must end in a status message |
| `scripts/design-shots.mjs` | Playwright Chromium + Chrome's fake camera: screenshots every designed state, camera-on included, into `docs/images/redesign/` (`GPU=1 node scripts/design-shots.mjs`) |
| `scripts/make-poster.mjs` | regenerates `public/poster.webp` — a real demo-clip frame with the real skeleton, captured by driving this app |
...
The interface is **"Gym mirror"**: the video is the page, and the count, the verdict, the fault and the
placement hint live on the glass as a HUD. It is the PRIMARY direction from
`docs/design/app-directions.md` §5 in `KalpKan/portfolio`, built 2026-09-21.

| Document | What it is |
|---|---|
| [`docs/design/spec.md`](docs/design/spec.md) | the spec, locked **before** any code: the filled design-first prompt, the audit items it closes, the motion budget, the measured contrast, and what may not change |
| [`docs/design/plan.md`](docs/design/plan.md) | the task-by-task implementation plan |
| [`docs/design/DESIGN.md`](docs/design/DESIGN.md) | the system that shipped: tokens, type scale, component states, accessibility, contrast, and the do-not list |
| [`docs/design/baseline.md`](docs/design/baseline.md) | the pre-redesign numbers the rebuild had to reproduce |
| `docs/images/redesign/` | every state at 1440 and 390, camera-on states included |

Three things to know before changing it:

- **One accent.** `#ffe66d` means *a rep was counted* — the count, the primary button, the 180 ms frame
  pulse. Nothing else. `--good` / `--bad` / `--paused` never leave the stage.
- **One added typeface**, JetBrains Mono 400/600, latin, self-hosted in `public/fonts/` under the SIL
  Open Font License 1.1. Everything else is the system stack.
- **Eight animations**, each listed in `DESIGN.md` §2 with its reason, one easing family, and a complete
  static state under `prefers-reduced-motion: reduce`.

`index.html`, `src/style.css`, `src/main.ts` and `src/draw.ts` are the view. `src/repCounter.ts`,
`src/form.ts`, `src/tracker.ts`, `src/features.ts`, `src/hints.ts` and `src/pose.ts` are the judgement,
and a design change must never touch them.
