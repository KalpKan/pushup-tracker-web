...
//   MIRROR=1 node scripts/make-mjpeg.mjs [id ...]   # flip-<id>.mjpeg: the same clip flipped horizontally, i.e. the
//                                                   # visitor facing the other way (TEST r3 D1, 2026-09-19)
...
for (const clip of gt.clips) {
  ...
  const out = join(outDir, `${mirror ? "flip-" : ""}${clip.id}.mjpeg`);
  ...
  execFileSync(ffmpeg, ["-y", "-loglevel", "error", "-i", src, "-f", "lavfi", "-i", "color=black:s=640x360:r=30:d=3",
    "-filter_complex", `[0:v]scale=640:360:force_original_aspect_ratio=decrease,pad=640:360:(ow-iw)/2:(oh-ih)/2,${mirror ? "hflip," : ""}fps=30,format=yuvj420p[a];[a][1:v]concat=n=2:v=1:a=0`,
    "-c:v", "mjpeg", "-q:v", "6", "-an", out]);
  console.log(`${mirror ? "flip-" : ""}${clip.id}.mjpeg`);
}
