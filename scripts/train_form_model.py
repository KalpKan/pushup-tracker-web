"""Train a form classifier (the retired v4 experiment) on the site's own landmarks. EXPERIMENT ONLY: the page
has not shipped a classifier since 2026-09-19 (FIX r3, b622fa3 removed src/classifier.ts, src/formFeatures.ts,
tests/classifier.test.ts and public/models/form-v*/); every output of this script is an artefact under OUT=,
nothing here is read by the site or by npm test. See README "Repeating the classifier experiment".
...
Input features (scripts/form_features.py; the browser port src/formFeatures.ts was removed with the classifier): the 12 landmarks' x, y
...
them for the accuracy number. Output, all experiment artefacts next to OUT (default scripts/form_v4, git-ignored):
OUT.h5 (Keras 2), OUT_scaler.json (StandardScaler mean/scale for eval_form_model.py), OUT_probs.json (60 held-out
frames with their Keras probability; the port-fidelity test that once read this file was removed with the
classifier) and OUT_report.json. No page file is written and there is no model URL to bump: if a classifier were
ever shipped again it would need a versioned, immutable-safe path (see incidents 2026-09-19, pushups TEST r2 D1).

usage: .venv/bin/python scripts/train_form_model.py     (env: SOURCES, HOLD_OUT, RANDOM_HOLD_OUT, AUG_*, ENSEMBLE, EPOCHS, OUT, TS_OUT)
"""
...
# TS_OUT=<path>: also write the scaler as a TypeScript module (the shape src/scaler.ts had while the site shipped a classifier). Off by default.
scaler_ts = f"""// GENERATED by scripts/train_form_model.py — do not edit by hand.
// StandardScaler of the {N_FEATURES} form features (scripts/form_features.py, FEATURES={FEATURES}) fitted on the training clips' lower-half frames
...
"""
if os.environ.get("TS_OUT"): open(os.environ["TS_OUT"], "w").write(scaler_ts)
# 60 held-out frames with the Keras probability: an experiment artefact (the port-fidelity test that compared
# these with the browser's numbers, tests/classifier.test.ts, was removed with the classifier).
pick = np.linspace(0, len(Xte) - 1, 60).astype(int)
json.dump({"note": f"{N_FEATURES} form features (scripts/form_features.py, {FEATURES}) and Keras P(good) of {OUT}.h5; generated by scripts/train_form_model.py",
           "frames": [{"features": [float(v) for v in Xte[i]], "prob": float(p[i]), "label": float(yte[i])} for i in pick]},
          open(os.environ.get("PROBS_OUT", f"{OUT}_probs.json"), "w"))
...
print(f"saved {OUT}.h5, {OUT}_scaler.json, {OUT}_probs.json, {OUT}_report.json" + (f", {os.environ['TS_OUT']}" if os.environ.get("TS_OUT") else ""))
